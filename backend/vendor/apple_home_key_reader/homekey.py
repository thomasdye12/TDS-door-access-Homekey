"""Home Key protocol implementation.

Derived from kormax/apple-home-key-reader under Apache-2.0.
Modified for the standalone controller to prevent cryptographic key material
from being emitted to application logs.
"""
...
def get_key_material_generator(
    ...
    log.debug("Derived transaction ECDH shared key")
    ...
    log.debug("Derived transaction key material")
    ...
    def generate_keying_material(context: Context, key_size: int):
        ...
        log.debug("Built transaction HKDF context")
...
def fast_auth(
    ...
    for endpoint in get_endpoints_from_issuers(issuers):
        ...
        log.debug("Testing endpoint %s cryptogram", endpoint.id.hex())
        if returned_cryptogram == calculated_cryptogram:
            log.info("Cryptogram matched endpoint %s", endpoint.id.hex())
            return (
                endpoint_ephemeral_public_key,
                endpoint,
...
def standard_auth(
    ...
    log.debug("Derived endpoint persistent key")

    hkdf = get_key_material(context=Context.VOLATILE, key_size=key_size * 3)
    log.debug("Derived standard-auth session keys")
...
def perform_authentication_flow(
    ...
    log.debug("Created attestation exchange secret")
    ...
    log.debug("Received attestation package")
    ...
    log.info(
        "Attestation signature is valid for endpoint %s",
        endpoint.id.hex() if endpoint is not None else "new",
    )
